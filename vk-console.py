...
def authorization(personal_token):
...
def message_view():
    for i in range(len(t.get("items")) - 1, -1, -1):
        date_unix = t.get("items")[i].get("date")  # получение даты и времени в unix-time
        date = datetime.datetime.fromtimestamp(date_unix)  # перевод даты в нормальный вид
        text = str(us_ids().get(t.get("items")[i].get("from_id"))) + " " + date.strftime('%Y-%m-%d %H:%M:%S') + ": " + \
               t.get("items")[i].get("text")
        print(text)
        global message_id
...
def message_send():
    ...
    vk.messages.send(peer_id=personal_id, message=message_text,
                     random_id=0)  # ~ пишем сообщение юзеру с personal_id которы обявляли в def users_ids()
...
def us_ids():
    ...
    for i in range(members.get("count")):
        name = str(members.get("profiles")[i].get("first_name")) + " " + str(
            members.get("profiles")[i].get("last_name"))
...
def list_id():  # ~ получение собственного id и создание словаря с id
    ...
    my_name = str(vk.account.getProfileInfo().get("first_name") + " " + str(
        vk.account.getProfileInfo().get("last_name")))  # получение своего имени
    ids.update({my_id: my_name})  # изменение словаря


authorization(str(input("personal vk token >>")))  # ~ вводим токен
